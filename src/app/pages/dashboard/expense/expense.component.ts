...
export class ExpenseComponent {
    ...
    isDataFetched = false; // Flag to track data fetch completion
  ...
  -2023/08/02 */
     ...
     */
        ...
        this.sumMoneyOutMonths = Array.from({ length: 4 }, (_, i) => {
            ...
            return filteredPrevMonthData.reduce(
    ...
    createChart(...sumMoneyOutMonths: number[]) {
        ...
        const prev1MonthName = new Date(
            new Date().setMonth(new Date().getMonth() - 1)
        ).toLocaleString('default', { month: 'long' });
        const prev2MonthName = new Date(
            new Date().setMonth(new Date().getMonth() - 2)
        ).toLocaleString('default', { month: 'long' });
        const prev3MonthName = new Date(
